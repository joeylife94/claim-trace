...
This is an integration acceptance harness, not a benchmark. It exercises the
existing ClaimTrace product path with deterministic fake embedding/LLM providers
while the document bytes, extraction, parsing, persistence and source locators are
real. Source PDFs are supplied by the workflow and are never committed here.
"""
...
def test_real_public_corpus_controlled_pilot(indexing_client: TestClient) -> None:
    ...
    if not corpus_dir_raw or not manifest_raw or not out_raw:
        pytest.skip("D3 real-public corpus paths are supplied only by the dedicated acceptance workflow")
    ...
    try:
        ...
        for item in documents:
            ...
            for claim in claims:
                ...
                assert _resolve_spans(indexing_client, document_id, claim["source_spans"]) == claim["text"]
            ...
            evidence["documents"].append(doc_evidence)
        ...
        indexing_client.app.state.llm_provider = FakeLLMProvider(
            structured_text=draft_json(
                [("선택된 공개 문서의 저장된 청구항 텍스트를 근거로 확인 가능한 내용을 제시한다.", ("EV-001",))]
            )
        )
